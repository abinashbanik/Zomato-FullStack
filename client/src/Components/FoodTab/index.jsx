        ...
        <>
            <div className="lg:hidden bg-white shadow-lg p-3 fixed bottom-0 z-10 w-full flex items-center justify-between md:justify-evenly text-gray-600 border">
                ...
                <Link to={`/${items.id}`}>   
                    <div className={
                        type === items.id
                          ? "flex flex-col relative items-center text-sm text-zomato-400 "
                          : "flex flex-col items-center text-sm "
                      }>
                        <div className={ type === items.id &&"absolute -top-3 transition duration-700 ease-in-out w-full h-2 border-t-2 border-zomato-400"}/>
                        {items.icon}
                        <h5 className="lext-lg">{items.name}</h5>
                    </div>
                </Link> 
                ))};
            </div>
...
const LargeTab =() => {
    ...
    return (
        <>
            <div className="hidden lg:flex container px-20 gap-14 my-8 mx-auto">
                {allTypes.map((items) => (
                    <Link to={`/${items.id}`}>
