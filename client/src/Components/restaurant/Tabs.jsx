...
const Tab = (props) => {
    ...
    return (
        <Link to={`/restaurant/${id}/${props.route}`} >
            ...
            <div className={classnames("text-gray-500 font-light border-zomato-400",{
                    "text-zomato-400 font-semibold ": props.isActive, })}>
                {/* <div className="absolute h-2 w-full border-b-2 border-zomato-400" /> */}
                <h3 className="text-lg  md:taxt-2xl">{props.title}</h3>
            </div>
        </Link>
    );
};
...
const TabsContainer = () => {
    ...
    return (
        <>
            <div className="flex relative items-center gap-8 md:gap-20 pb-4 overflow-x-scroll border-b-2">
                {tabs.map((tab) => (
